...
export async function GET(
  ...
  const student = await db.student.findFirst({
    ...
    include: {
      school: true,
      campus: true,
      attendanceEntries: { where: { register: { termId } }, select: { status: true } },
      resultEntries: {
        where: { sheet: { termId, status: { in: ["PUBLISHED", "LOCKED"] } } },
        include: {
          sheet: {
            include: {
  ...
  let y = 790;
  page.drawText(student.school.name, { x: 45, y, size: 16, font: bold, color: red });
  y -= 24;
  page.drawText("Student Report Card", { x: 45, y, size: 19, font: bold });
  y -= 24;
  const term = student.resultEntries[0]!.sheet.term;
  page.drawText(`${student.campus.name} | ${term.academicSession.name} | ${term.name}`, { x: 45, y, size: 10, font: regular });
  y -= 28;
  page.drawText(`Student: ${student.lastName}, ${student.firstName}`, { x: 45, y, size: 11, font: bold });
  page.drawText(`Admission no.: ${student.admissionNumber}`, { x: 330, y, size: 10, font: regular });
  y -= 28;
  ...
  y -= 10;
  page.drawLine({ start: { x: 45, y }, end: { x: 550, y }, thickness: 1, color: red });
  y -= 18;
  const totals: number[] = [];
  for (const entry of student.resultEntries.sort((a, b) => a.sheet.subject.name.localeCompare(b.sheet.subject.name))) {
    const components = entry.sheet.components;
    if (!components.every((component) => component.scores[0])) continue;
    const total = totalWeightedScore(components.map((component) => ({ score: component.scores[0]!.score, maxScore: component.maxScore, weight: component.weight })));
    totals.push(total);
    const grade = resolveGrade(total, entry.sheet.gradingScheme.bands);
    page.drawText(entry.sheet.subject.name.slice(0, 35), { x: 45, y, size: 9, font: regular });
    page.drawText(total.toFixed(2), { x: 280, y, size: 9, font: regular });
    page.drawText(grade?.label ?? "—", { x: 345, y, size: 9, font: bold });
    page.drawText((grade?.remark ?? "—").slice(0, 22), { x: 410, y, size: 9, font: regular });
    y -= 19;
  }
  y -= 12;
  ...
  page.drawText(`Average: ${average.toFixed(2)}%`, { x: 45, y, size: 11, font: bold });
  page.drawText(`Attendance: ${attendance.attendanceRate}% (${attendance.total} days)`, { x: 280, y, size: 11, font: bold });
  y -= 45;
  page.drawText("This report includes only published or locked subject results.", { x: 45, y, size: 9, font: regular, color: rgb(0.35, 0.38, 0.42) });
