...
CREATE TABLE "public"."pilot_runs" (
  ...
  "name" TEXT NOT NULL,
  "status" "public"."PilotStatus" NOT NULL DEFAULT 'PLANNED',
...
CREATE TABLE "public"."pilot_checklist_items" (
  ...
  "area" TEXT NOT NULL,
  "label" TEXT NOT NULL,
  "status" "public"."PilotChecklistStatus" NOT NULL DEFAULT 'NOT_STARTED',
  "evidence" TEXT,
  "verifiedById" TEXT,
...
CREATE TABLE "public"."pilot_issues" (
  ...
  "route" TEXT,
  "severity" "public"."PilotIssueSeverity" NOT NULL,
  "status" "public"."PilotIssueStatus" NOT NULL DEFAULT 'OPEN',
  "resolution" TEXT,
...
CREATE TABLE "public"."launch_approvals" (
  "id" TEXT PRIMARY KEY,
  "pilotRunId" TEXT NOT NULL,
  "status" "public"."LaunchApprovalStatus" NOT NULL DEFAULT 'DRAFT',
  "summary" TEXT,
  "approvedById" TEXT,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM "public"."pilot_runs" r
    WHERE r.id=NEW."pilotRunId" AND r."schoolId"=NEW."schoolId"
  ) THEN
    RAISE EXCEPTION 'pilot issue run scope mismatch';
  END IF;
  ...
  IF NEW."campusId" IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'pilot issue campus scope mismatch';
  END IF;
...
CREATE TRIGGER "pilot_issues_scope_guard"
BEFORE INSERT OR UPDATE ON "public"."pilot_issues"
FOR EACH ROW EXECUTE FUNCTION "public"."enforce_pilot_scope"();
